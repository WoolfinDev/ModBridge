...
def open_folder(path: Path) -> Tuple[bool, Optional[str]]:
    """Reveal a folder in the OS file manager."""
    try:
        path.mkdir(parents=True, exist_ok=True)
    except Exception as e:
        logger.error(f"Failed to prepare folder {path}: {e}")
        return False, str(e)
    path_str = str(path)
    if sys.platform == 'win32':
        try:
            os.startfile(path_str)  # noqa: S606 - local path by construction
            return True, None
        except Exception as e:
            logger.error(f"Failed to open folder: {e}")
            return False, str(e)

    # Linux: xdg-open is standard but may be missing (minimal WMs)
    # or broken (no DBus session / bad default handler), so fall back
    # through desktop-specific openers, then plain file managers.
    ...
    for manager in ('nautilus', 'dolphin', 'thunar', 'pcmanfm', 'nemo', 'caja'):
        candidates.append([manager, path_str])
    ...
    for cmd in candidates:
        ...
        try:
            ...
            last_err = str(e)
            logger.warning("Opener %s failed for %s: %s", cmd[0], path_str, e)
    if not tried_any:
        last_err = "no file manager found (xdg-open/gio/nautilus/...)"
        logger.error("Failed to open folder %s: %s", path_str, last_err)
